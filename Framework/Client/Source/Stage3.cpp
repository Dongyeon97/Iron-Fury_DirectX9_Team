...
void CStage3::Initialize()
{
	...
	m_wpPlayer.lock()->SetActorPosition({ 0.0f, 20.0f, -430.0f });
...
void CStage3::BeginPlay()
{
	...
	// m_wpPlayer.lock()->GetSoundComponent().lock()->PlaySoundByDistance("bossish1", ESoundType::BGM);
	NonDestroyData tNonDestroyData = CGameManager::GetInstance()->GetNonDestroyData();

	m_wpLoverboy.lock()->SetSavedBulletHolding(tNonDestroyData.prLoverboy.first);
	m_wpLoverboy.lock()->SetSavedBulletLoaded(tNonDestroyData.prLoverboy.second);

	m_wpDisperser.lock()->SetSavedBulletHolding(tNonDestroyData.prDisperser.second);
	m_wpDisperser.lock()->SetSavedBulletLoaded(tNonDestroyData.prDisperser.second);

	m_wpPenetrator.lock()->SetSavedBulletHolding(tNonDestroyData.prPenetrator.second);
	m_wpPenetrator.lock()->SetSavedBulletLoaded(tNonDestroyData.prPenetrator.second);

	m_wpChainGun.lock()->SetSavedBulletHolding(tNonDestroyData.prChainGun.second);
	m_wpChainGun.lock()->SetSavedBulletLoaded(tNonDestroyData.prChainGun.second);
